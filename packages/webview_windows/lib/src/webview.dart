...
class WebviewController extends ValueNotifier<WebviewValue> {
  ...
  /// Moves the cursor then applies wheel delta in one platform call.
  Future<void> _scrollAt(Offset position, Offset scrollDelta) async {
    if (_isDisposed) {
      return;
    }
    assert(value.isInitialized);
    return _methodChannel.invokeMethod('scrollAt', [
      position.dx,
      position.dy,
      -scrollDelta.dx,
      -scrollDelta.dy,
                ...
                ? Listener(
                    ...
                    onPointerSignal: (signal) {
                      if (signal is PointerScrollEvent) {
                        // Win the pointer-signal race against ancestor
                        // Scrollables (home page SingleChildScrollView).
                        GestureBinding.instance.pointerSignalResolver.register(
                          signal,
                          (PointerSignalEvent event) {
                            final scroll = event as PointerScrollEvent;
                            _controller._scrollAt(
