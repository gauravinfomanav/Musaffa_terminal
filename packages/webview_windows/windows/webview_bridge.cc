...
constexpr auto kMethodScrollAt = "scrollAt";
...
void WebviewBridge::HandleMethodCall(
  ...
  // scrollAt: [double x, double y, double dx, double dy]
  if (method_name.compare(kMethodScrollAt) == 0) {
    const flutter::EncodableList* list =
        std::get_if<flutter::EncodableList>(method_call.arguments());
    if (!list || list->size() != 4) {
      return result->Error(kErrorInvalidArgs);
    ...
    const auto x = std::get_if<double>(&(*list)[0]);
    const auto y = std::get_if<double>(&(*list)[1]);
    const auto dx = std::get_if<double>(&(*list)[2]);
    const auto dy = std::get_if<double>(&(*list)[3]);
    if (!x || !y || !dx || !dy) {
      return result->Error(kErrorInvalidArgs);
    }
